...
public class AaaKafkaIntegration {
    ...
    private JsonNode serializeStat(AuthenticationStatisticsEvent event) {
        ...
        authMetricsEvent.put(AUTH_SUCCESS_TRANS, event.subject().getAuthSuccessTrans());
        authMetricsEvent.put(AUTH_FAILURE_TRANS, event.subject().getAuthFailureTrans());
        authMetricsEvent.put(START_REQ_TRANS, event.subject().getStartReqTrans());
        authMetricsEvent.put(EAP_PKT_TX_AUTH_EAP, event.subject().getEapPktTxauthEap());
        authMetricsEvent.put(TRANS_RESP_NOT_NAK, event.subject().getTransRespNotNak());
